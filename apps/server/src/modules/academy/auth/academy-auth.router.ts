...
router.post('/google', async (req, res, next) => { try { const result = await service.loginWithGoogle(req.body.credential); cookie(res, result.refreshToken); res.json({ data: { accessToken: result.accessToken, user: result.user } }); } catch (error) { next(error); } });
