...
const findReusablePending = async (userId: string, courseId?: string, bundleId?: string) => {
  const order = await prisma.academyOrder.findFirst({
    where: { userId, courseId: courseId ?? null, bundleId: bundleId ?? null, status: 'PENDING', checkoutExpiresAt: { gt: new Date() } },
...
const createOrderAndSession = async (userId: string, input: CheckoutInput, target: { courseId?: string; bundleId?: string; title: string; description: string; price: number; accessDays?: number | null }) => {
  ...
  const reusable = await findReusablePending(userId, target.courseId, target.bundleId);
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ quantity: 1, price_data: { currency: 'pln', unit_amount: Math.round(target.price * 100), product_data: { name: target.title, description: target.description.slice(0, 500) } } }],
...
const sendConfirmationIfNeeded = async (orderId: string) => {
  ...
  const title = order.course?.title ?? order.bundle?.title ?? 'Kurs Akademii';
...
const fulfill = async (orderId: string, session: Stripe.Checkout.Session) => {
  const order = await prisma.academyOrder.findUnique({ where: { id: orderId }, include: { course: true, bundle: { include: { courses: { include: { course: true } } } } } });
  if (!order) return;
  ...
  if (order.status !== 'PAID') {
    ...
    const enrollments = order.courseId && order.course
...
const markRefund = async (paymentIntent: string, amount: number, reason = 'Zwrot płatności') => {
  const order = await prisma.academyOrder.findUnique({ where: { stripePaymentId: paymentIntent }, include: { user: true, course: true, bundle: { include: { courses: true } } } });
  if (!order) return;
  ...
  const otherPaidOrders = full && courseIds.length ? await prisma.academyOrder.findMany({
    ...
    select: { courseId: true, bundle: { select: { courses: { select: { courseId: true } } } } },
  }) : [];
  const retained = new Set(otherPaidOrders.flatMap((item) => item.courseId ? [item.courseId] : item.bundle?.courses.map((course) => course.courseId) ?? []));
...
const orderSelect = { id: true, status: true, amount: true, currency: true, paidAt: true, refundedAt: true, refundAmount: true, invoiceRequested: true, invoiceNumber: true, invoiceUrl: true, createdAt: true, course: { select: { title: true, slug: true } }, bundle: { select: { title: true, slug: true } } } as const;
...
export const adminListOrders = (filters: { status?: string; search?: string }) => prisma.academyOrder.findMany({
  ...
  include: { user: { select: { id: true, name: true, email: true } }, course: { select: { title: true } }, bundle: { select: { title: true } }, events: { orderBy: { createdAt: 'desc' }, take: 10 } },
  orderBy: { createdAt: 'desc' },
  take: 500,
});
