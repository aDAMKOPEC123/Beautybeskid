...
export const sendCampaignTest=async(data:any,email:string)=>{const subject=clean(data.subject,160),content=clean(data.content,5000);if(!subject||!content)throw new AppError('Uzupełnij temat i treść',400);await sendEmail(email,`[TEST] ${subject}`,`<div style="white-space:pre-wrap">${escapeCampaign(content)}</div>`);return{sent:true};};
...
export const initializeAcademyMarketingScheduler=()=>{const run=()=>processAcademyEmailQueue().catch(error=>prisma.academyOperationalEvent.create({data:{severity:'ERROR',source:'EMAIL_QUEUE',message:error instanceof Error?error.message:'Błąd kolejki'}}).catch(()=>undefined));void run();return setInterval(run,60_000);};
