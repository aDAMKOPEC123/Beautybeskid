...
export const runAcademyAutomations = async () => {
  ...
  for (const enrollment of enrollments) {
    ...
    if (age > 5 * 3_600_000) await deliver({ userId: enrollment.userId, courseId: enrollment.courseId, type: 'START_GUIDE', email: enrollment.user.email, subject: `Jak zacząć: ${enrollment.course.title}`, html: `<h1>Twój kurs jest gotowy</h1><p>Zacznij od pierwszej lekcji i ucz się we własnym tempie.</p><p><a href="${url}">Rozpocznij kurs „${title}”</a></p>` });
