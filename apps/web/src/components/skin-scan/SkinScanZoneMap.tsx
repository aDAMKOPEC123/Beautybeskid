...
import type { SkinScanAnalysis } from '@/api/skin-scans.api';
...
export const SkinScanZoneMap = ({ analysis, className }: Props) => {
    ...
    <section className={`rounded-3xl border border-border bg-white shadow-sm ${className ?? ''}`}>
      <div className="p-5 sm:p-7">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-heading text-xl font-semibold text-[#1A3828]">Mapa stref twarzy</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Analiza przebarwień i zaczerwienienia w poszczególnych strefach twarzy.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {ZONE_ORDER.map((zoneKey) => {
            ...
            return (
              <div
                key={zoneKey}
                className="rounded-2xl border border-border/70 bg-[#FAF9F6] p-4"
              >
