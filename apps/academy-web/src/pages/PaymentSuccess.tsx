...
export function PaymentSuccess() {
  ...
  if (query.data?.status === 'PAID') return <div className="academy-profile-empty"><CheckCircle2 className="text-green-600" /><h1>Płatność potwierdzona</h1><p>Dostęp został nadany. Potwierdzenie zamówienia wysłaliśmy na Twój adres e-mail.</p>{query.data.postPurchaseOffer&&<div className="academy-post-purchase-offer"><strong>10% na kolejny kurs</strong><p>Kod: <b>{query.data.postPurchaseOffer.code}</b></p><small>Ważny do {new Date(query.data.postPurchaseOffer.endsAt).toLocaleDateString('pl-PL')}</small><Link to="/">Wybierz kolejny kurs</Link></div>}<Link to="/moje-kursy">Przejdź do mojej nauki</Link></div>;
