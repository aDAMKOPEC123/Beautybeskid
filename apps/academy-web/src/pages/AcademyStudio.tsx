...
export function AcademyStudio() {
  ...
  const emptyDraft = { title: '', description: '', difficulty: 'BEGINNER', estimatedMinutes: 60, price: 0, compareAtPrice: null as number | null, isFree: false, accessDays: null as number | null, isFeatured: false, isBestseller: false, isComingSoon: false, previewLessonId: '', instructorName: 'Wiktoria Ćwik', instructorBio: '', bundleGroup: '', tagsText: '', thumbnailUrl: '', status: 'DRAFT' };
  ...
  const saveCourse = useMutation({ mutationFn: () => { const { tagsText, ...data } = draft; const payload = { ...data, bundleGroup: data.bundleGroup || null, instructorBio: data.instructorBio || null, tags: tagsText.split(',').map(tag => tag.trim()).filter(Boolean) }; return selected ? academyApi.adminUpdateCourse(selected.id, { ...payload, slug: selected.slug }) : academyApi.adminCreateCourse({ ...payload, slug: slugify(draft.title) }); }, onSuccess: (course: any) => { setSelectedId(course.id ?? selectedId); refresh(); } });
  ...
  const choose = (course: any) => { setSelectedId(course.id); setDraft({ ...emptyDraft, title: course.title, description: course.description, difficulty: course.difficulty, estimatedMinutes: course.estimatedMinutes, price: Number(course.price || 0), compareAtPrice: course.compareAtPrice ? Number(course.compareAtPrice) : null, isFree: course.isFree, accessDays: course.accessDays, isFeatured: course.isFeatured, isBestseller: course.isBestseller, isComingSoon: course.isComingSoon, previewLessonId: course.previewLessonId || '', instructorName: course.instructorName || 'Wiktoria Ćwik', instructorBio: course.instructorBio || '', bundleGroup: course.bundleGroup || '', tagsText: (course.tags || []).join(', '), thumbnailUrl: course.thumbnailUrl ?? '', status: course.status }); };
