...
export function CheckoutPage({ type = 'course' }: { type?: 'course' | 'bundle' }) {
  ...
  return <div className="academy-checkout-page">
    ...
    {!user?.emailVerified&&<div className="academy-readiness-warning"><strong>Potwierdź adres e-mail</strong><p>Potwierdzenie jest potrzebne do zakupu.</p><button type="button" onClick={()=>academyApi.resendVerification().then(()=>toast.success('Wysłaliśmy nowy link')).catch((err:any)=>toast.error(err?.response?.data?.message||'Nie udało się wysłać linku. Spróbuj ponownie za chwilę.'))}>Wyślij link ponownie</button></div>}
