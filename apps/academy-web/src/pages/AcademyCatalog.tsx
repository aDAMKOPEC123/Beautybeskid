...
export function AcademyCatalog() {
  ...
  const { data: courses = [], isLoading: coursesLoading } = useQuery({ queryKey: ['academy', 'courses', isAuthenticated], queryFn: isAuthenticated ? academyApi.getCourses : academyApi.getPublicCourses });
  const hasAccess = user?.role === 'ADMIN' || (courses as any[]).length > 0;
  ...
  return <div className="academy-page space-y-10">
    ...
    <section className="academy-hero">
      ...
      <div className="relative z-[1] max-w-2xl">
        ...
        <div className="academy-hero-actions"><a href="#kursy" className="academy-button academy-button-light">Odkryj kursy <ArrowRight className="w-4 h-4" /></a>{started[0] && <Link to={`/kurs/${started[0].slug}`} className="academy-text-button">Kontynuuj naukę</Link>}{!user && <Link to="/logowanie" className="academy-text-button">Zaloguj się</Link>}</div>
      </div>
