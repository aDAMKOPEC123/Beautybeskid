...
export function AcademyProfile() {
  ...
  const { data: courses = [] } = useQuery({ queryKey:['academy','profile-courses'], queryFn: academyApi.getCourses, enabled: isAuthenticated });
  const { data: certificates = [] } = useQuery({ queryKey:['academy','profile-certificates'], queryFn: academyApi.getCertificates, enabled: isAuthenticated });
